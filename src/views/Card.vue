<template>
  <v-app>
    <v-container id="card">
      <v-flex v-for="question in questions" :key="question.question" class="card">
        <v-card>
          <v-card-title>{{ question.question }}</v-card-title>
          <v-radio-group v-model="question.answer" row class="flex-center">
      ...
      <h1>this is the sum {{ summeAllerFragen }}</h1>
    </v-container>
  </v-app>
</template>
